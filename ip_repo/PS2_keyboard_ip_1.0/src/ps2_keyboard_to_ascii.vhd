      ...
      CASE state IS
        ...
        CASE ps2_code IS
          WHEN x"58" =>                   --caps lock code
                ...
                END IF;
                state <= translate;
          WHEN x"12" => shift_l <= NOT break; -- Left Shift
                        state <= ready;
          WHEN x"59" => shift_r <= NOT break; -- Right Shift
                        state <= ready;
          WHEN x"14" => -- Control key
            IF e0_code = '1' THEN
              control_r <= NOT break; -- Right Control
            ELSE
              control_l <= NOT break; -- Left Control
            END IF;
                    state <= ready;
          WHEN OTHERS => state <= translate; -- Continue with translation
        END CASE;
          END IF;
